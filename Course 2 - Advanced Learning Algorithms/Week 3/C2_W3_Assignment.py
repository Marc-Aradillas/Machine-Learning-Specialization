...
def eval_mse(y, yhat):
    ...
    ### START CODE HERE ###
        ...
        """
        The eval_mse function takes in two numpy arrays, y and yhat, both of shape (m,) 
        or (m,1), where m is the number of examples. It calculates the mean squared 
        error between the two arrays, which is a measure of how well the predicted 
        values match the true values.
        """
        err_i = ((yhat[i] - y[i])**2)
        err += err_i
    err = err / (2*m)
    ...
    """
    The function first initializes the error, err, to zero. It then loops through each 
    example and calculates the error for that example using the formula (yhat[i] - y[i])**2. 
    The calculated error for each example is added to the total error err. After the loop, 
    the total error is divided by 2*m to calculate the mean squared error.

    The function then returns the calculated mean squared error, err.
    """
    ### END CODE HERE ### 
...
def eval_cat_err(y, yhat):
    ...
    ### START CODE HERE ### 
        """
        The function eval_cat_err takes in two numpy arrays y and yhat and calculates the 
        categorization error between the two arrays.

        The function first determines the number of elements in the input arrays, m. Then, it 
        initializes a counter incorrect to zero. It then loops through each element in the arrays 
        using the range function and the m variable. For each element, if the predicted value 
        yhat[i] is not equal to the target value y[i], the counter incorrect is incremented by 1.

        After the loop, the categorization error cerr is calculated by dividing the total number 
        of incorrect predictions by the total number of examples m. The cerr value is then returned.

        The categorization error is a metric commonly used in classification tasks to evaluate 
        the performance of a classifier. It represents the proportion of examples that were 
        classified incorrectly out of the total number of examples.
        """
        if yhat[i] != y[i]: 
            incorrect += 1
    cerr = incorrect/m  
...
model.compile(
    ...
    # code defines a neural network model using the Keras API in TensorFlow.
        
        # The model architecture consists of three fully connected layers, with 120, 40, and classes neurons 
        # respectively, where classes is the number of output classes. The activation function for the first 
        # two layers is ReLU, while the output layer uses a linear activation function. The Sequential function 
        # from Keras is used to define the model as a sequence of layers.
        
        # The model is compiled using the compile function, specifying the loss function and optimizer to be 
        # used during training. The loss function is set to SparseCategoricalCrossentropy, which is commonly 
        # used for multiclass classification problems. The optimizer is set to Adam with a learning rate of 0.01.
        
        # The model is then trained using the fit function, specifying the training data (X_train and y_train) 
        # and the number of epochs to train for (1000). During training, the weights of the model are adjusted 
        # to minimize the specified loss function using the optimizer. The random seed is set to 1234 using 
        # tf.random.set_seed at the beginning of the code, which ensures that the results are reproducible.
...
model_s.compile(
    ...
    # In the first layer, we have six units with ReLU activation function, and in the second layer, 
    # six units with a linear activation function. The loss function is SparseCategoricalCrossentropy, 
    # and we use Adam optimization algorithm with a learning rate of 0.01
...
model_r.compile(
    ...
    # the model consists of a dense layer with 120 units, relu activation, and a kernel regularizer 
    # with a regularization parameter of 0.1, followed by a dense layer with 40 units, relu activation, 
    # and a kernel regularizer with a regularization parameter of 0.1, followed by a dense layer with 6 units 
    # and a linear activation. The model is then compiled using a loss function of SparseCategoricalCrossentropy 
    # with from_logits=True and an Adam optimizer with a learning rate of 0.01.
)
