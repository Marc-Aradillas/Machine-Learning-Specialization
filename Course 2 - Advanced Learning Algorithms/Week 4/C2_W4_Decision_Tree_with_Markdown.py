...
def compute_entropy(y):
    ...
    The compute_entropy function takes in an array y that indicates whether each 
    example at a node is edible (1) or poisonous (0). It calculates the entropy 
    at that node using the formula:

    "entropy = -p_1 log_2(p_1) - p_0 log_2(p_0)"

    where 'p_1' is the proportion of edible examples at the node, and 'p_0' is the 
    proportion of poisonous examples at the node. If all the examples at the node 
    are of the same class, the entropy is zero.
    """
    ...
    p1 = np.sum(y == 1) / len(y) # proportion of edible examples
    p0 = np.sum(y == 0) / len(y) # proportion of poisonous examples
    
    if p0 == 0 or p1 == 0: # if all examples are of the same class
        return 0
...
def split_dataset(X, node_indices, feature):
    ...
    This implementation iterates over each index in node_indices 
    and checks the value of the feature at that index. If the 
    feature value is 1, the index is added to the left indices 
    list, otherwise it's added to the right indices list. The 
    left and right indices lists are then returned at the end.
    """
    ...
    for i in node_indices: # for loop to check each value in node
        if X[i][feature] == 1: # if node is 1 set left
...
def compute_information_gain(X, y, node_indices, feature):
    ...
    It seems that there is an issue with the implementation Exercise 1 compute_entropy() function. 
    The value of the information gain is nan, which means there might be a division by zero or a log of 
    zero happening which was found in the calculation for entropy, the wrong function was built.
