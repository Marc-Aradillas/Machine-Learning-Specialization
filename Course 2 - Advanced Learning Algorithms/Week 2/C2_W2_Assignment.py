...
def my_softmax(z):  
    ...
    """    
    ### START CODE HERE ### 
    N = len(z) # length for value z
    a = np.zeros(N) # numPy tool zeroes
    ez_sum = 0
    for k in range(N):
        ez_sum += np.exp(z[k]) # numPy tool
    for j in range(N):
        a[j] = np.exp(z[j])/ez_sum #numPy tool, j instead of k for this loop.
    return a
...
model = Sequential(
    [               
        ### START CODE HERE ### 
        tf.keras.layers.Input(shape=(400,)), # layer 'units' parameter, specifies the number of neurons in the layer | changed from Dense to Input for tf.keras.layer code.
        Dense(25, activation='relu', name = "L1"), # rely not an activation function in Keras, should be relu.
