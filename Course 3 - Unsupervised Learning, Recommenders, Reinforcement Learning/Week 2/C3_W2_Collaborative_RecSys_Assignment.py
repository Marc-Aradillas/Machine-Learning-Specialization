...
def cofi_cost_func(X, W, b, Y, R, lambda_):
    ...
    ### START CODE HERE ###         
    for j in range(nu):
        w = W[j, :]  # Get the j-th row of W
        b_j = b[0, j]  # Get the j-th element of b
        for i in range(nm):
            x = X[i, :]  # Get the i-th row of X
            y = Y[i, j]  # Get the i-th movie rating by the j-th user
            r = R[i, j]  # Get the rating indicator (1 if rated, 0 otherwise)
            J += np.square(r * (np.dot(w, x) + b_j - y))  # Accumulate the cost
            
    J = J / 2  # Divide the cost by 2 (to match the squared difference formula)
