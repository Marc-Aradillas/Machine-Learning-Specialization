...
def find_closest_centroids(X, centroids):
    ...
    """
    The function computes the distance between the example and every centroid 
    using the inner loop for j in range(centroids.shape[0]). The distance 
    between an example X[i] and a centroid centroids[j] is calculated using 
    the linalg.norm() function from the numpy module. The Euclidean distance 
    is used as the distance metric, which is the default behavior of the linalg.norm() 
    function when the ord=None argument is passed.

    The computed distances are stored in a temporary array distance. Once the distance 
    between the example and every centroid has been computed, the function finds the 
    index of the centroid with the smallest distance using the np.argmin() function, 
    which returns the index of the minimum value in an array.

    Finally, the function assigns the index of the closest centroid to the idx array 
    for the current example using the statement idx[i] = np.argmin(distance).
...
def compute_centroids(X, idx, K):
    ...
    ### START CODE HERE ###
    """
     loops over each centroid i from 0 to K-1. For each centroid, it extracts 
     all the data points in X that are assigned to that centroid (i.e., those 
     for which idx == i). It then computes the mean of these data points along 
     each feature using np.mean(ci, axis=0), where ci is the matrix of data 
     points assigned to centroid i. This gives the new centroid for centroid i, 
     which is stored in the centroids array.
     
    """
    for i in range(K):
        # Get all the examples assigned to the ith centroid
        ci = X[idx == i, :]
        
        # Compute the new centroid as the mean of the examples assigned to it.
