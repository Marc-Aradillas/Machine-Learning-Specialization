...
array is often referred to as the dimension though mathematicians may prefer rank. The vector shown has a dimension of 𝑛. The elements of a vector
...
from 0 to n-1. In notation, elements of a vector, when referenced individually will indicate the index in a subscript, for example, the 0𝑡ℎ element, 
...
indices. NumPy starts indexing at zero so the 3rd element of an vector 𝐚 is a[2].
"""
...
# These operators work on an element-by-element basis. For example: 𝑐𝑖=𝑎𝑖+𝑏𝑖
...
Using a for loop, implement a function which returns the dot product of two vectors. 
																							 ...
																							 n-1
The function to return given inputs 𝑎 and 𝑏: 𝑥 = ∑ (𝑎𝑖𝑏𝑖)
																							 i=0
...
In Course 1, 2-D matrices are used to hold training data. Training data is 𝑚 examples by 𝑛 features creating an (m,n) array. Course 1 does not 
